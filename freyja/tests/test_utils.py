...
class UtilsTests(unittest.TestCase):
    def setUp(self):
        ...
        self.usher_barcodes = pd.read_csv('freyja/data/usher_barcodes.csv',
                                          index_col=0)
        indexSimplified = [dfi.split('_')[0]
                           for dfi in self.usher_barcodes.index]
        self.usher_barcodes = self.usher_barcodes.loc[indexSimplified, :]
        self.depth = pd.read_csv(
            'freyja/data/test.depth', header=None, sep='\t', index_col=1)
