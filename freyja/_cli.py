    ...
    if '--barcodes' in sys.argv:
        ...
        if pathogen in pathogens:
            ...
            if pathogen == "SARS-CoV-2":
                with open(
                    os.path.join(locDir, 'data', 'last_barcode_update.txt'),
                    'r'
                ) as f:
                ...
                click.echo(os.path.join(
                    locDir, 'data', 'usher_barcodes.feather'
                ))
                ctx.exit()
                ...
                other_version_path = os.path.join(
                    locDir, 'data', 'last_barcode_update_other.txt'
                )
                ...
                if os.path.isfile(barcode_path):
                    ...
                    raise click.ClickException(
                        f"Barcode file '{barcode_filename}' does not exist. "
                        "Please use the 'update' command to download the barcode first."
                    )
                ctx.exit()

        else:
            click.echo(
                'Pathogen not in available list (see freyja-barcodes repo). '
